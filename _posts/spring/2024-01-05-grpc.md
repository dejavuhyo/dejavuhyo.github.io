...
이는 [프로토콜 버퍼](https://www.baeldung.com/google-protocol-buffer)를 사용하여 `.proto` 파일에서 수행된다. 또한 페이로드 메시지의 구조를 설명하는 데에도 사용된다.
...
### 2) Maven Plugin 사용
gRPC는 Maven 빌드 시스템을 위한 protobuf-maven-plugin을 제공합니다.
...
os-maven-plugin extension/plugin `${os.detected.classifier}`와 같은 다양하고 유용한 플랫폼 종속 프로젝트 속성을 생성한다.
